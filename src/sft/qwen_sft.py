...
def apply_chat_template(example, tokenizer):
    ...
    if example["data_type"] == "vanilla_benign":
        if "unsafe" in example["gpt-response"].split('<Answer>')[1]:
            response = example["completion"]
        else:
            response = example["gpt-response"]
    elif example["data_type"] == "vanilla_harmful":
        if "unsafe" in example["gpt-response"].split('<Answer>')[1]:
            response = example["gpt-response"]
        else:
            response = example["completion"]
    elif example["data_type"] == "adversarial_benign":
        if "unsafe" in example["gpt-response"].split('<Answer>')[1]:
            response = example["completion"]
        else:
            response = example["gpt-response"]
    elif example["data_type"] == "adversarial_harmful":
        if "unsafe" in example["gpt-response"].split('<Answer>')[1]:
            response = example["gpt-response"]
        else:
            response = example["completion"]
    example["text"] = f'''<|im_start|>user{instruction}<|im_end|>\n<|im_start|>assistant{response}<|im_end|>\n<|endoftext|>'''
    return example
